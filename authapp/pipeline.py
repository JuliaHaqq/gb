...
def save_user_profile(backend, user, responce, *args, **kwargs):
    if backend.name != 'vk-oauth2':
        return

    url_method = 'https://api.vk.com/method/'
    access_token = responce.get('access_token')
    fields = ','.join(['bdate', 'sex', 'about', 'photo_max_orig'])



    api_url = f'{url_method}users.get?fields={fields}&access token={access_token}&v=5.131'
    ...
    data_json = responce.json()['response'][0]
